...
public class WikiDetailController {
    ...
    public String wikiDetail(Model model, @RequestParam("id") int id, HttpSession session) {
        ...
        System.out.println("선택한 메뉴트리 작성자 유저번호: " + dao.getMenuDetail(id).getUserno());
        ...
        menuDirection.add(dao.getMenuDetail(id).getTitle()); // 작성자부터 제목을 넣고 시작
        ...
        String guide = "작성자 : " + userdto.getNickname(); 
        
        if (wikiData == null) {
            System.out.println("초기값을 제공하겠습니다.");
	...
	public ResponseEntity<String> saveEditorData(@RequestBody String editorData, HttpSession session) {
	    ...
	    try {
	        if (dao.getData(wikiId) == null) {
                ...
                // 알림을 보낼 팔로워 리스트 가져오는 작업
	            List<String> followers = adao.sortMyFollower(userno);
                // 알림을 구독자에게 추가
	            BoardDTO bdto = dao.getPostInfo(wikiId+""); 
                String message = "작성자 : " + bdto.getNickname() + " 님이 새로운 게시물을 작성하였습니다. (" + bdto.getTitleShare() + ")";
	...
	public ResponseEntity<HashMap<String, Object>> uploadFile2(
        ...
        Integer wikiId = (Integer) session.getAttribute("WikiId"); // 이 부분은 위키 ID에 따라 변동 처리
